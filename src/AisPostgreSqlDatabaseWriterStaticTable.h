#ifndef AisPostgreSqlDatabaseWriterStaticTable_h
#define AisPostgreSqlDatabaseWriterStaticTable_h
...
#include <AisWriterStaticTable.h>
...
*/
class AisPostgreSqlDatabaseWriterStaticTable : public aisWriterStaticTable{
public:
	AisPostgreSqlDatabaseWriterStaticTable(std::string username, std::string password, std::string hostname, std::string databaseName, std::string tableName, int iterations = 100000)
	{
	...
	~AisPostgreSqlDatabaseWriterStaticTable()
	{
		aisDebug("AisPostgreSqlDatabaseWriterStaticTable Destructor");
		if(m_initialized)
		{
			try
			{
	...
	bool writeEntry(const AisMessage& message)
	{
		try
		{
			...
			string command = "INSERT INTO '" + m_tableName + "'" + " VALUES(DEFAULT, "  +
