#ifndef AisPostgreSqlDatabaseWriterRadarSingleAISTable_h
#define AisPostgreSqlDatabaseWriterRadarSingleAISTable_h
...
*/
class AisPostgreSqlDatabaseWriterRadarAISTable : public AisWriter{
public:
	AisPostgreSqlDatabaseWriterRadarAISTable(std::string username, std::string password, std::string hostname, std::string databaseName, 
		std::string tableName, int iterations, bool create_RadarTable, bool create_AISTable):
		...
		m_sqlStatement("")
	{
		...
		if (create_RadarTable)
		{
			m_createTable = createRadarTable(tableName);
		} else if (create_AISTable)
		{
			m_createTable = createTable(tableName);
		}
	}
	
	~AisPostgreSqlDatabaseWriterRadarAISTable()
	{
		aisDebug("AisPostgreSqlDatabaseWriterRadarAISTable Destructor");
		if(m_initialized)
		{
			try
			{
	...
	bool writeEntryRadar(const RadarMessage& message)
	{
		try
		{	
			if(m_currentIteration == 1 || m_iterations <= 0)
			{
				m_sqlStatement = "INSERT INTO " +  m_tableName + " (key_column,messagetype, mmsi, sog, lon, lat, cog, true_heading, datetime, imo, streamid, target_status, target_acq)";
				m_sqlStatement += " VALUES(DEFAULT, ";
			} else
			{
				m_sqlStatement+= ", (DEFAULT, ";
			}
			m_sqlStatement+=
				boost::lexical_cast<std::string>(message.getMESSAGETYPE()) + ", " +
				boost::lexical_cast<std::string>(message.getTRACKNUM())+ ", " +
				boost::lexical_cast<std::string>(message.getSOG())+ ", " +
				boost::lexical_cast<std::string>(message.getLON())+ ", " +
				boost::lexical_cast<std::string>(message.getLAT())+ ", " +
				boost::lexical_cast<std::string>(message.getCOG())+ ", " +
				...
				boost::lexical_cast<std::string>(message.getTRACKNUM())+ ", '" +
				sanitize(boost::lexical_cast<std::string>(message.getSTREAMID()))+ "', '" +
				sanitize(boost::lexical_cast<std::string>(message.getTARGETSTATUS()))+ "', '" +
				sanitize(boost::lexical_cast<std::string>(message.getTARGETACQ()))+ "')";
			if(m_currentIteration++ == m_iterations || m_iterations <= 0)
			{
				m_currentIteration = 1;
...
private:
	...
	bool createTable(string tableName)
	{
		...
		try
		{
			m_sqlStatement = "CREATE TABLE IF NOT EXISTS " + tableName;
			...
			m_sqlStatement += "CONSTRAINT " + tableName + "_pkey PRIMARY KEY (key_column))";
			StatementExecutor statementExecutor(m_sqlStatement);
			m_con->perform(statementExecutor);
			m_sqlStatement = string("");
		}
	...
	bool createRadarTable(string tableName)
	{
		...
		try
		{
			...
			m_sqlStatement += "posfixtype double precision, ";
			m_sqlStatement += "streamid character varying(36) DEFAULT NULL::character varying, ";
			m_sqlStatement += "target_status character varying(36) DEFAULT NULL::character varying, ";
			m_sqlStatement += "target_acq character varying(36) DEFAULT NULL::character varying, ";
			m_sqlStatement += "CONSTRAINT " + tableName + "_pkey PRIMARY KEY (key_column))";
			StatementExecutor statementExecutor(m_sqlStatement);
			m_con->perform(statementExecutor);
			m_sqlStatement = string("");
